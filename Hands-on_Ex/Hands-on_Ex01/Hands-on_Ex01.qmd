...
This hands on exercise will involve going through the ggplot packages of R and applying the principle of Layered Grammar of Graphics. By the end, I will be able to apply the essential graphical elements provided by ggplot2 to create elegant and yet functional statistical graphics.
...
Calling the ggplot() function initializes a ggplot object and a blank canvas will appear. The data argument is used to definte the dataset for plotting.
...
[`geom_violin`](https://ggplot2.tidyverse.org/reference/geom_violin.html) is designed for creating violin plot. Violin plots are a way of comparing multiple data distributions. With a violin plot, it's easier to compare several distributions since they're placed side by side.
...
The code chunk below adds mean values using *geom\_()* function.
...
To display different subset of the data in one plot, facetting can be used to generate small multiples (trellis plot).
