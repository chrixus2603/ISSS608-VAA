...
In this hands-on exercise, I will learn how to plot data visualisation for visualising correlation matrix with R. It consists of three main sections. First is how to create correlation matrix using pairs() of R Graphics. Next is to plot corrgram using corrplot package of R; and lastly to create an interactive correlation matrix using plotly R.
...
There are more than one way to build scatterplot matrix with R. Let's learn how to create a scatterplot matrix by using the pairs function of R Graphics.
...
airs function of R Graphics provided many customisation arguments. For example, it is a common practice to show either the upper half or lower half of the correlation matrix instead of both. This is because a correlation matrix is symmetric.
...
![With corrplot package, we can use the *cor.mtest()* to compute the p-values and confidence interval for each pair of variables.](images/unnamed-chunk-19-1.png){width="540"}
